...
function myjudge(outputFile) {
    ...
    const match = outputFile.match(/CPU OPT:\s*[\d.]+\s*ms,\s*([\d.]+)\s*GFLOPS/);
    const gflops = match ? parseFloat(match[1]) : null;
    points['cpu opt'][0] = gflops;
}

function judge(outputFile){
    let start = outputFile.indexOf('start---cpu-opt');
    let end = outputFile.indexOf('end---cpu-opt', start);
    myjudge(outputFile.substring(start + 'start---cpu-opt'.length, end));
    return points;
} 
